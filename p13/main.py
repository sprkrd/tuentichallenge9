...
def findMinimumGold(goal_char_name, goal_skills):
    minimum_gold = {}
    all_skill_subsets = [p for p,_ in getAllPartitions(goal_skills, 2)]
    ...
    for char in tqdm.tqdm(reachable_characters):
        char_skills = char.skills&goal_skills
        minimum_gold[(char.name, char_skills)] = char.cost
        for subchar_name0, subchar_name1 in fusions[char.name]:
            subchar0 = char_index[subchar_name0]
            subchar1 = char_index[subchar_name1]
            for skills0 in all_skill_subsets:
                for skills1 in all_skill_subsets:    
                    combined_skills = char_skills|skills0|skills1
